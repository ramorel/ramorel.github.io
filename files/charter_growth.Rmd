...
p1 <- 
  ...
  scale_fill_distiller(name = "", palette = "Spectral",  limits = c(0, 1)) + 
  ggtitle("Proportion of students enrolled in charter schools by county, 2016") +
  theme_void()
...
p2 <- 
  ...
  mutate(
    cate_prop = case_when(
      prop > 0.95 ~                "Above 95%",
      between(prop, 0.75, 0.949) ~  "Between 75% and 95%",
      between(prop, 0.5, 0.749) ~   "Between 50% and 75%",
      between(prop, 0.25, 0.49) ~   "Between 25% and 50%",
      between(prop, 0.10, 0.249) ~  "Between 10% and 25%",
      between(prop, 0.01, 0.1) ~    "Betweee 1% and 10%",
      prop < 0.01 ~                 "Under 1%"
      )
    ) %>% 
  mutate(cate_prop = factor(cate_prop, levels = c("Above 95%", 
                                                  "Between 75% and 95%", 
                                                  "Between 50% and 75%", 
                                                  "Between 25% and 50%",
                                                  "Between 10% and 25%",
...
**Please note**: It takes ~one hour to pull the data from the CCD API!
